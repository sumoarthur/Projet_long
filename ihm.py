...
def planning_cours(canvas,matiere, notion):
    ...
    print(niveau)
    graphe=creation_arbre()
    L_parcours = parcours(graphe, niveau)
    L_parcour_avec_exo=defintion_exercice(L_parcours, niveau)



    L_dipso.sort()
    for k in range(len(L_dipso)):
        canvas.itemconfig(L_dipso[k]+1, text=L_parcour_avec_exo[k].nom)
