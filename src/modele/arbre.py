...
class Arbre :
    ...
    def _innerdot(self) :
        ...
        if self.racine.niveau=='Debutant':
            colors=str('green')
        elif self.racine.niveau=='Intermediaire':
            colors=str('blue')
        else:
            colors=str('red')


        if type(self.racine) == Cours:
            shape = ""
        else:
            shape = "diamond"
        ...
        stri+='\"{label}\"  [label=\"{infos}\", style = filled, peripheries = 1, shape = \"{shape}\" ,\
                fillcolor = {col}, color = \"{colors}\"];\n'.format(label=id(self),infos=infos,col=col,colors=colors,shape=shape)
        for a in self :
            stri+=a._innerdot()
        return stri
